...
constexpr auto usage = "usage: wordcount_cpp [--json] [--top N] [--max-word N] <file>";
...
[[nodiscard]] auto parse_args(int argc, char** argv) -> Options {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (arg == "--json") {
            options.json = true;
        } else if (arg == "--top" || arg == "--max-word" || arg == "--bench-runs" ||
                   arg == "--bench-warmups") {
            if (++index >= argc) {
...
[[nodiscard]] auto read_file(const std::string& path) -> std::vector<unsigned char> {
...
[[nodiscard]] auto estimated_unique_words(const std::vector<unsigned char>& bytes) -> std::size_t {
...
[[nodiscard]] auto count_words(const std::vector<unsigned char>& bytes,
    ...
    for (auto& [entry_word, count] : counts) {
        entries.push_back({.word = entry_word, .count = count});
    }
    ...
    return {.total = total, .unique = counts.size(), .top = std::move(entries)};
}

void render_json(const Result& result) {
    std::print(R"({{"total":{},"unique":{},"top":[)", result.total, result.unique);
    auto separator = std::string_view{};
    for (const auto& entry : result.top) {
        std::print(R"({}{{"word":"{}","count":{}}})", separator, entry.word, entry.count);
        separator = ",";
    }
    std::println("]}}");
}
...
[[nodiscard]] auto mix_byte(std::uint32_t checksum, unsigned char byte) -> std::uint32_t {
    return (checksum ^ static_cast<std::uint32_t>(byte)) * checksum_prime;
}

[[nodiscard]] auto mix_u32(std::uint32_t checksum, std::uint32_t value) -> std::uint32_t {
    for (auto index = 0; index < 4; ++index) {
        checksum = mix_byte(checksum, static_cast<unsigned char>(value & 0xffU));
        value >>= 8U;
    }
    return checksum;
}

[[nodiscard]] auto mix_u64(std::uint32_t checksum, std::uint64_t value) -> std::uint32_t {
...
void render_bench(const std::vector<unsigned char>& bytes, const Options& options) {
    ...
    for (std::size_t index = 0; index < options.bench_runs; ++index) {
        checksum_value = mix_u32(checksum_value,
                                 checksum(count_words(bytes, options.top, options.max_word)));
    }
    const auto elapsed =
            std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - started);

    std::println(R"({{"mean_ms":{:.6f},"checksum":{}}})",
